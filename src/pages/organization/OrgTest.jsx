...
function OrgTest() {
    ...
    const fetchDynamicView = async () => {
        try {
            ...
            // Debug: Log the actual columns and first contact data
            console.log('📋 View columns:', data.viewMetadata?.columns?.map(col => `${col.name} (${col.type})`))
            if (data.contacts && data.contacts.length > 0) {
                console.log('👤 First contact data:', data.contacts[0])
                console.log('🏢 Parent customer expanded:', data.contacts[0].parentcustomerid)
            }
    ...
    const handleEditContact = (contact) => {
        ...
        navigate(`/orgtest/edit/${contact.contactid}`)
    }
    ...
    const DynamicTable = ({ viewMetadata, data }) => {
            ...
            <div className="bg-white shadow rounded-lg overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-gray-200">
                    <h3 className="text-lg font-medium text-gray-900">
                        {viewMetadata.displayName || 'Dynamic Contact View'}
                    </h3>
        ...
        const formatCellValue = (value, type, contact, fieldName) => {
            ...
            if (type === 'lookup' || fieldName === '_parentcustomerid_value' || fieldName === 'parentcustomerid') {
                console.log(`🔍 Lookup field debug - Field: ${fieldName}, Type: ${type}, Value:`, value, 'Contact:', contact)
            }
            ...
            if (fieldName === 'parentcustomerid' && contact._parentcustomerid_value) {
                ...
                console.log(`🔗 Field mapping: ${fieldName} -> _parentcustomerid_value =`, actualValue)
            }
        ...
        // Separate function to handle different lookup field types
        const formatLookupField = (fieldName, value, contact) => {
            ...
            if (fieldName === '_parentcustomerid_value' || fieldName === 'parentcustomerid') {
                console.log(`🏢 Customer lookup value:`, value, 'Contact data:', contact)
                ...
                if (contact.parentcustomerid) {
                    ...
                    console.log(`🔍 Enriched parent customer:`, parentCustomer)
                    ...
                    if (parentCustomer.entityType === 'account') {
                        ...
                        customerType = parentCustomer.name ? 'Account' : 'Contact'
                    }
            ...
            // Handle other lookup field types (extend here for future lookups)
            // Example patterns for future expansion:
            // - _ownerid_value -> systemuser or team
            // - _createdby_value -> systemuser
            // - Custom lookups -> custom entities
                    ...
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                                Testing dynamic Dataverse view integration
                            </p>
                        </div>
                        ...
                        {!loading && viewData && (
                            <div className="mt-6 bg-gray-100 rounded-lg p-4">
                                ...
                                <div className="text-xs text-gray-600 space-y-1">
                                    <div>View Name: {viewData.name}</div>
                                    <div>Columns: {viewData.columns?.length || 0}</div>
                                    <div>Records: {contacts.length}</div>
                                    <div>View ID: {viewData.savedqueryid?.substring(0, 8)}...</div>
                                </div>
                            </div>
                        )}
                    </div>
