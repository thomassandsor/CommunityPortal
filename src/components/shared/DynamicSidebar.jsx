...
function DynamicSidebar() {
    ...
    const fetchMenuItems = async (clearCache = false) => {
        ...
        if (loading && menuItems.length > 0 && !clearCache) {
            return
        }
        ...
        try {
            ...
            const url = clearCache 
                ? '/.netlify/functions/entity-config?clearCache=true'
                : '/.netlify/functions/entity-config'
    ...
    const getEntityIcon = (iconName) => {
        const iconMap = {
            ...
            'office-building': (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 21l18 0M5 21V7l8-4v18M9 9h1m0 0h1m-1 0v1m0-1V8M9 17h1m0 0h1m-1 0v1m0-1v-1" />
                </svg>
            ),
            'home': (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            'briefcase': (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2-2v2m8 0H8m8 0v2a1 1 0 01-1 1H9a1 1 0 01-1-1V6m8 0H8" />
                </svg>
            ),
            'currency-dollar': (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    return (
        <div className={`${isCollapsed ? 'w-16' : 'w-64'} bg-white shadow-lg flex flex-col h-screen overflow-hidden transition-all duration-300 ease-in-out`}>
            {/* Logo/Header */}
                    ...
                    <div className="flex items-center space-x-2">
                        {!isCollapsed && (
                            <button
                                ...
                                title="Refresh menu (clear cache)"
                                disabled={loading}
                            >
                                <svg 
                        ...
                        {menuItems.map((item) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center ${isCollapsed ? 'px-2 justify-center' : 'px-4'} py-3 text-sm font-medium rounded-lg transition-colors ${
                                        isActive
                                ...
                                >
                                    <span className={`${isCollapsed ? 'mr-0' : 'mr-3'} flex-shrink-0`}>
                                        {item.icon}
                                    </span>
                                    {!isCollapsed && (
                                        <>
                                            ...
                                            {item.requiresAdmin && (
                                                <span className="ml-auto text-xs text-red-500 flex-shrink-0" title="Admin Only">
                                                    🔑
                                                </span>
                                            )}
                                        </>
            ...
            <div className="flex-shrink-0 border-t px-4 py-4 bg-gray-50">
